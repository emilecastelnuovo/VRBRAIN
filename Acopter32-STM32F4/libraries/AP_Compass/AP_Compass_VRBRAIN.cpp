...
/*
 *       AP_Compass_VRBRAIN.cpp - Arduino Library for HMC5843 I2C magnetometer
...
bool AP_Compass_VRBRAIN::read_register(uint8_t address, uint8_t *value)
{
    if (_i2c->readRegister((uint8_t)COMPASS_ADDRESS, address, value) != 0) {
        _healthy[0] = false;
        return false;
    }
    return true;
...
bool AP_Compass_VRBRAIN::write_register(uint8_t address, uint8_t value)
{
    if (_i2c->writeRegister((uint8_t)COMPASS_ADDRESS, address, value) != 0) {
        _healthy[0] = false;
        return false;
    }
    return true;
...
// Read Sensor data
bool AP_Compass_VRBRAIN::read_raw()
{
    uint8_t buff[6];
 ...
 */
bool AP_Compass_VRBRAIN::re_initialise()
{
